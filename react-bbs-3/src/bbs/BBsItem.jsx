...
class BBsItem extends Component {
  ...
  render() {
    ...
    return (
      <tr key={bbsVO.id}>
        <td className="text-center">{bbsVO.bbsDate}</td>
        <td className="text-center">{bbsVO.bbsAuth}</td>
        <td
          style={titleStyle}
...
/*
  현재 이(BBsItem) Component는
  Router의 1대 자손이 아니다.
  1대 자손이 아닐 때는
  this.props.history 객체를
    직접 전달받지 못한다.
  1대 자손이 아닌 Component에서
...
/*
  ...
  2. 이벤트 핸들러 등록
    ...
    = 함수()
    으로 등록을 하게 되면
    각 Component가 rendering 되는 동안 이벤트 핸들러도
    등록하는 것이 아니라 함수를 호출하여 실행해 버린다.

    그런데 이벤트 핸들러에게 어떤 값을 전달하여
    그 값을 핸들러 코드내에서 사용하고느 할때는 ?
