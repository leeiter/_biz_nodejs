    ...
    */
    ...
    router.get("/searchStation", function(req, res) {
        /*
            ...
            {칼럼명 : RegExp(값, 'ig')} : 중간문자열검사 SLQ에서 %값%
        */
    ...
    router.get("/viewStation", function(req, res) {
        ...
        gjStation.count({}).exec(function(err, count) {
            ...
            // 전달하면
            // 페이지를 그리는데 필요한 정보가 담긴 pageArray 객체를
            //      자동으로 생성해준다.
            ...
            // exec : 함수를 실행하라. 원래는 then이였는데 버전 update가 되면서 바뀌었다.
            // limit : 100개만 보여줘라.
            // skip :  100를 건너뛴 다음부터 보여줘라.
            
            // 실제 한페이지에 보여줄 데이터를 읽어오는데(select, find)
            // 위에서 계산 pageLimit, offset값을 활용하여
            // 필요한 데이터만 추출한다.
            gjStation.find({}).limit(pageLimit).skip(offset).exec(function(err, stations) {
                // 데이터 리스트 페이지에
                // 지금까지 생성한 값들을
                // 전달하여 데이터리스트를 보이고
                // 하단에 페이지 리스트를 보여서
